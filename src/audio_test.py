...
class AudioTester:
    ...
    def list_audio_devices(self):
        ...
        for i in range(device_count):
            ...
            print(
                f"  Host API: {self.audio.get_host_api_info_by_index(device_info['hostApi'])['name']}"
            )
            print()
    ...
    def test_microphone_capture(self, device_id=None, duration=10):
        ...
        try:
            ...
            for _ in range(int(self.sample_rate / self.chunk_size * duration)):
                ...
                print(
                    f"\rRecording... {elapsed:.1f}s / {duration}s", end="", flush=True
                )
    ...
    def test_system_audio_capture(self, device_id=None, duration=10):
        ...
        print(
            "Note: You may need to configure a virtual audio device for this to work."
        )
    ...
    def test_dual_capture(self, mic_device=None, system_device=None, duration=10):
        ...
        print(
            "⚠️  Note: This is a basic test. Full dual capture requires more complex implementation."
        )
...
def main():
    ...
    try:
        ...
        if not input_devices:
            ...
            print(
                "Please check the AUDIO_SETUP.md file for detailed setup instructions."
            )
            print("\nCommon solutions:")
        ...
        while True:
            ...
            if choice == "1":
                if not input_devices:
                    print(
                        "❌ No input devices available. Please connect a microphone first."
                    )
                    print("See AUDIO_SETUP.md for detailed instructions.")
                else:
                    device_id = input(
                        "Enter microphone device ID (or press Enter for default): "
                    ).strip()
                ...
                device_id = input(
                    "Enter system audio device ID (or press Enter for default): "
                ).strip()
                device_id = int(device_id) if device_id.isdigit() else None
                duration = input(
                    "Enter recording duration in seconds (default 10): "
