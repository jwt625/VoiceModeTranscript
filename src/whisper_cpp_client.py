...
class WhisperCppClient:
    ...
    def _test_connection(self):
        ...
        try:
            ...
            print(f"⚠️  Warning: Could not connect to whisper.cpp server at {self.server_url}")
            print(f"   Error: {e}")
            print(f"   Make sure the server is running with: ./whisper.cpp/build/bin/whisper-server")
    ...
    def transcribe_file(self, file_path: str, 
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"🎤 Whisper.cpp result: '{formatted_result['text']}' "
                      f"(confidence: {formatted_result['confidence']:.2f}, "
                      f"time: {processing_time:.2f}s)")
                
                return formatted_result
            else:
                print(f"❌ Whisper.cpp server error: {response.status_code} - {response.text}")
    ...
    def _audio_data_to_temp_file(self, audio_data: bytes, 
                                sample_rate: int = 16000, 
    ...
    def _format_result(self, whisper_result: Dict[str, Any], 
        ...
        try:
            ...
            return {
                ...
                'detected_language_probability': whisper_result.get('detected_language_probability', 0.0),
                'is_final': True,
                'segments': segments,
            ...
            if segments:
                ...
                for segment in segments:
                    ...
                    if 'avg_logprob' in segment:
                        ...
                        segment_confidence = max(0.1, min(1.0, (avg_logprob + 1.0) * 0.5 + 0.5))
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.total_requests + self.failed_requests > 0:
            success_rate = self.total_requests / (self.total_requests + self.failed_requests)
