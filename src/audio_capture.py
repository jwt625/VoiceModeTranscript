...
class AudioCapture:
    ...
    def start_recording(self, session_id, callback=None):
        ...
        self.recording_thread = threading.Thread(
            target=self._recording_loop, args=(session_dir,), daemon=True
        )
        ...
        try:
            ...
            while self.is_recording:
                try:
                    ...
                    if self.mic_stream:
                        try:
                            ...
                            if self.callback and chunk_count % 10 == 0:
                                ...
                                self.callback(
                                    mic_data, source="microphone", audio_level=mic_level
                                )

                            # Process microphone audio for transcription
                            if (
                                self.callback and chunk_count % 50 == 0
                            ):  # Process every 50th chunk (~3 seconds)
                    ...
                    if self.system_stream:
                        try:
                            ...
                            if (
                                ...
                                self.callback(
                                    audio_chunk, source="system", is_transcription=True
                                )
                    ...
                    save_interval = (
                        self.sample_rate // self.chunk_size * 10
                    )  # 10 seconds
    ...
    def _init_streams(self):
        ...
        try:
            ...
            print(
                f"✅ Microphone stream initialized (device: {self.mic_device_id or 'default'})"
            )
        ...
        if self.system_device_id is not None:
            try:
                ...
                print(
                    f"✅ System audio stream initialized (device: {self.system_device_id})"
                )
