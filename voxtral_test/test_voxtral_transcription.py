...
def test_voxtral_transcription():
    ...
    try:
        ...
        print("   vllm serve mistralai/Voxtral-Mini-3B-2507 --tokenizer_mode mistral --config_format mistral --load_format mistral")
...
def test_voxtral_server_connection():
    ...
    try:
        ...
        print(f"❌ vLLM server not running or not accessible: Connection failed")
        print("💡 To start the server, run:")
        print("   vllm serve mistralai/Voxtral-Mini-3B-2507 --tokenizer_mode mistral --config_format mistral --load_format mistral")
    except Exception as e:
