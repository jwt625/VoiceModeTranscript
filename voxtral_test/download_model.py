...
def check_disk_space(required_gb=10):
    ...
    try:
        ...
        if free_gb >= required_gb:
            print(f"✅ Disk space check: {free_gb:.1f} GB available (need ~{required_gb} GB)")
            return True
        else:
            print(f"❌ Insufficient disk space: {free_gb:.1f} GB available (need ~{required_gb} GB)")
            return False
    except Exception as e:
...
def main():
    ...
    if success_count == len(files_to_download):
        ...
        print(f"   The script will automatically detect and use the local model")
    else:
        print(f"\n❌ Download incomplete ({success_count}/{len(files_to_download)} files)")
        print(f"💡 You can run this script again to resume the download")
