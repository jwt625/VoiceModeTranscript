...
def test_voxtral_understanding():
    ...
    try:
        ...
        text_chunk = TextChunk(
            text="What is the main topic of this speech? Who is the speaker?"
        )
        user_msg = UserMessage(content=[audio_chunk, text_chunk]).to_openai()
        ...
        text_chunk = TextChunk(
            text="Please provide a brief summary of this audio in one sentence."
        )
        user_msg = UserMessage(content=[audio_chunk, text_chunk]).to_openai()
...
def test_multilingual_capabilities():
    ...
    if len(audio_files) == 1:
        print(
            "💡 Only JFK sample available. For multilingual testing, add more audio files to:"
        )
        print(f"   {samples_dir}")
        print(
